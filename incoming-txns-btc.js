...
async function checkStatusByConfirmBlock(latestBlock) {
    var toconfirm = latestBlock - 6;
    await update_db("trade_receive", 'status=0 AND block IS NOT NULL AND cur_type="BTC"', 'confirmation=' + latestBlock + "-block");
    ...
    if (checkTrx.length > 0) {
        checkTrx.forEach(async function(row) {
            var txnid = row.txid;
            await update_db("trade_receive", 'txid="' + txnid + '" AND cur_type="BTC"', 'status=1');
        });
    }
}

btc_blocks();
async function btc_blocks() 
{
  var data = "*";
  ...
  for (var i = StartingBlock; i <= newBlockNumber; i++) 
  {
   ...
   for (var j = 0; j < arrayLength; j++) {
    ...
    { 
       ...
       if (checkTrx.length==0){
         var checkAddress = await select_db("address", "*", "address='" + toaddress + "' AND currency='1'",'');
         if (checkAddress.length > 0) {
          console.log("Txn Incoming");
		  var checkCurrency = await select_db("currency", "*", "id='" + 1 + "' AND status='1'", '');
  ...
  await update_db("latestblock_btc", 'id=1', 'blockNumber='+i);
 }
 setTimeout(function(){
  btc_blocks();
 }, 1*60*1000);
